...
#endif
#include "yunyi/eventloop/INetEventLoop.h"
namespace yunyi {
class Session;
 ...
 */
class TransportCore : public INetEventLoop {
public:
    TransportCore();
    ~TransportCore() override;
     ...
     * @post IOCP 创建，工作线程启动
     */
    bool init(uint32_t ioThreads = 0) override;
     ...
     */
    SOCKET createListenSocket(uint16_t port, const char* bindIp = "::",
                              SOCKET* outIpv4Sock = nullptr) override;
     ...
     */
    bool startAccept(SOCKET ls, OnAcceptCallback cb) override;
     ...
     * @return true 成功投递，false 失败
     */
    bool startConnect(const char* host, uint16_t port, OnConnectCallback cb) override;
     ...
     */
    bool postRecv(SOCKET s, OnRecvCallback cb) override;
     ...
     */
    bool postSend(SOCKET s, const char* d, size_t len, OnSendCallback cb) override;
     ...
     */
    SOCKET createUdpSocket(uint16_t localPort = 0, const char* bindIp = "::") override;
     ...
     */
    bool postRecvFrom(SOCKET s, OnRecvFromCallback cb) override;
    ...
    bool postSendTo(SOCKET s, const char* d, size_t len,
                    const sockaddr_storage& peer, OnSendToCallback cb) override;
     ...
     */
    bool bindSocket(SOCKET s) override { return bindToIocp(s); }
     ...
     */
    void closeSocket(SOCKET s, OnCloseCallback cb, int err = 0) override;
