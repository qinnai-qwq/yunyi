 ...
 * @brief 控制连接状态机实现
 */
#include "yunyi/channel/ControlChannel.h"
#include "yunyi/protocol/FrameCodec.h"
#include "yunyi/net/NetUtil.h"
#include "yunyi/engine/Scheduler.h"
#include "yunyi/log/Logger.h"
#include "yunyi/session/Session.h"
#include "yunyi/tls/TlsPskContext.h"
...
void ControlChannel::startHeartbeat() {
    ...
    CC_LOG("[ControlChannel] Heartbeat started (" + std::to_string(kHeartbeatIntervalMs) + "ms)");
}
...
void ControlChannel::tryReconnect() {
    ...
    if (++_reconnectAttempts > kMaxReconnectAttempts) {
        CC_LOG("[ControlChannel] Max reconnect attempts reached, giving up");
        stopHeartbeat();
        setState(ControlState::Disconnected);
        return;
    }

    CC_LOG(std::string("[ControlChannel] Reconnect attempt ") + std::to_string(_reconnectAttempts)
           + "/" + std::to_string(kMaxReconnectAttempts) + " in "
           + std::to_string(kReconnectDelayMs) + "ms");
